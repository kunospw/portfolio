...
const GameMenu = ({ menuItems, onMenuItemClick, selectedMenuItem, onClose, onDragStart }) => {
  return (
    <div className="w-80 h-auto pixel-window-frame">
      <div className="pixel-content-area p-5">
        {/* Title bar */}
        <div 
          className="flex items-center justify-between pb-4 mb-4 border-b-2 border-[#a7b5ba] cursor-move"
          onMouseDown={onDragStart}
        >
          <h2 className="text-lg font-bold text-black select-none">Menu</h2>
          <button
            onClick={onClose}
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                onMouseDown={(e) => e.stopPropagation()}
                className={`flex items-center w-full text-left p-3 transition-all rounded-sm cursor-pointer ${
                  isSelected ? 'bg-black text-white' : 'hover:bg-blue-200 text-black'
                }`}
              >
                <span className="w-8 text-lg flex items-center">{item.icon}</span>
                <span className="text-base font-bold ml-2">{item.name}</span>
              </button>
            );
          })}
        </div>
